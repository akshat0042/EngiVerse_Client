                ...
                <div className=" flex flex-row h-full w-full z-10 ">
                    <div className={`w-20  h-screen bg-[#1E1F22] top-0 left-0 flex flex-col-reverse`}>
                        <div className=" h-[31.5%] pl-[0.45rem] pt-2 space-y-2">
                            ...
                            <button className={`w-12 text-[#60e43f] rounded-full flex h-12 bg-[#313338] justify-center items-center sidebar-icon`} onClick={openSetting}>
                                <img src={SelfPro(dp)} className="rounded-full sidebar-iconn"></img>
                            </button>
                        </div>
                        {isOpen && (
                  ...
                  <div className="absolute top-0 left-0 text-black mt-3 ml-14">
                    ...
                    <div className={"flex flex-col h-screen w-72 "}>
                        ...
                        <div className={"bg-[#2B2D31] h-full flex-col border-r border-[#2c2e30] overscroll-contain overflow-auto scrolling-touch"}>
                            {chat.map((data) => (<div className="yeetfont text-[#eaeaea] flex flex-row chat-item text-lg w-full mt-2 h-[3.2rem] p-1 rounded-md text-left" onClick={() => { handelChatClick(data._id, data.users[1].firstName) }}>
                                ...
                                <div className="ml-1 flex flex-col w-full">
                                    {data.isGroupChat?(<><div className="overflow-x-clip"> {data.chatName}</div></>):(<></>)}
                        ...
                        intro polls */}
                            ...
                            <div className="yeetfont text-[#eaeaea] chat-item mx-auto text-lg w-full mt-2 h-10 px-2 rounded-md text-left" onClick={()=>ChangeGenral(selectedCommunity._id)}>
                                General
                            </div>
                            <div className="yeetfont text-[#eaeaea] chat-item mx-auto text-lg w-full mt-2 h-10 px-2 rounded-md text-left" onClick={()=>getPolls(selectedCommunity._id)}>
                                Polls
                            </div>
                            <div className="text-center items-center origin-center justify-center  w-full">
                                <hr className="w-[13rem] border-[#949494] mx-auto" ></hr>
                            ...
                            <div className="yeetfont text-[#ff3c3c] chat-item mx-auto text-lg w-full mt-2 h-10 px-2 rounded-md text-left" onClick={()=>exitComm(selectedCommunity._id)}>
                                EXIT
                            </div>
                        </div></>)}
                    </div>
                    <div className={"flex flex-col w-screen"}>
                                            ...
                                            <div className="flex items-end">
                                                <div className="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-2 items-start">
                                                    <div className="flex flex-col">
                                                        <div className="h-[1rem] px-1 z-30 text-white rounded bg-[#1E1F22] mt-1 w-fit ">
                                                            {data.firstName}
                                                        </div>
                                                        <span
                        ...
                        {localStorage.getItem("comsele") ? (<div>
                            ...
                            {isBroadcast && (
                                ...
                                {general ? (
                                        <div className={" h-[92.9%] bg-[#cacaca] rounded-se-full  backdrop-blur-yeet hover:backdrop-blur-sm w-full flex flex-col-reverse p-3"}>
                                            <div>
                                                ...
                                                onChange={(event) => { setNewMsgGen({...newMsgGen,content: event.target.value }) }}
                                ...
                                {isPolls?(
                                    <div className={" h-[92.9%] bg-[#cacaca] rounded-se-full  backdrop-blur-yeet hover:backdrop-blur-sm w-full flex flex-col-reverse p-3"}>
                                        <button className="h-[3.5rem] w-full rounded-full bg-[#1F2937] text-white" onClick={()=>crtPolls()}>
                                            Create polls
                                        </button>
                                        <div className={"p-3 shadow-lg hover:shadow-2xl rounded-md transition-shadow mb-4 h-full w-full overflow-y-auto no-scrollbar"} ref={messagesRef}>
                                            <div className="max-w-md w-full rounded-md overflow-hidden shadow-lg bg-gray-800 text-white">
                                                <div className="px-6 py-4">
                                                    ...
                                                    {polls?(<>
                                                    {polls.map(poll => (
                                                            <div key={poll.id} className="max-w-md w-full rounded-md overflow-hidden shadow-lg bg-gray-800 text-white mb-4">
                                                                <div className="px-6 py-4">
                                                                    ...
                                                                    {poll.options.map(option => (
                                                                        <div key={option.id} className="mb-4">
                                                                            ...
                                                                            <span className="text-gray-300">{option.selected.length} votes</span>
            ...
            {addPolls ? (<div className={"fixed inset-0 flex items-end justify-end z-50 backdrop-blur-sm"}>
                <div className={" flex flex-col-reverse h-[29rem] w-[30rem] rounded-lg shadow-lg bg-[#2b384a]"} >
                    <div className="h-[3.5rem] p-1 flex flex-row-reverse bg-[#2b384a] rounded-b-lg ">
                        <button className="h-[3rem] bg-green-600 hover:bg-green-400 rounded-md w-[12rem] " onClick={()=>postPoll(selectedCommunity._id)}> POST </button>
                        <button className="h-[3rem] bg-slate-500 hover:bg-slate-400 rounded-md mr-1 w-[9rem]" onClick={crtPolls}>Cancel </button>
                        <div className="w-full flex flex-row">
                        {sendPoll.options.length < 6 && (
                                    <div>
                                        <button className=" h-[3rem] text-white bg-green-600 hover:bg-green-400 rounded-md w-[6rem]" onClick={handleAddOption}>Add Option</button>
                                    </div>
                                )}
                        </div>
                    </div>
                    <div className="h-full flex flex-col">
                        ...
                        <div className="h-full flex flex-col text-white">
                            <div className="h-[6rem] w-full flex flex-col text-left p-1 justify-center items-center">
                                <input  className="w-[20rem] rounded-md pl-2 bg-[#1F2937] h-[3.7rem]" placeholder="Context" value={sendPoll.context}  onChange={(event)=>{
                            ...
                            <div className="option h-full w-full flex flex-col mt-2 text-left p-1">
                                ...
                                {sendPoll.options.map((option, index) => (
                                    <div key={index}>
                                        ...
                                        />
                                        <button className="bg-red-600 p-1 ml-1 mt-1 rounded-md" onClick={() => handleRemoveOption(index)}>Remove</button>
                                    </div>
                                ))}
